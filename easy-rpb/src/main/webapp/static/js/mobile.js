...
if(typeof AndroidWebAppInterface != 'undefined'){
	...
	window.Rongyi = {
	    ...
	    gotoPictureDetail: function(data){
	    	AndroidWebAppInterface.HtmlNavigateToPictureDetail(data);
	    },
	    sendEmail: function(){
			return  AndroidWebAppInterface.HtmlNavigateToEmail();
		},
		gotoNewMap: function(latitude, longitude, logo, name){
			return  AndroidWebAppInterface.HtmlNavigateToNewMap(latitude, longitude, logo, name);
		},
	    type: 'Android'
	} 
	Rongyi.getCityName();
...
function defineIOS(bridge){
	...
	window.Rongyi = {
	    ...
	    sendEmail: function(){
	    	bridge.callHandler('email');
	    }, 
	    gotoPictureDetail: function(data){
	    	bridge.callHandler('HtmlNavigateToPictureDetail',{'data': data});
	    },
	    gotoNewMap: function(latitude, longitude, logo, name){
			return  bridge.callHandler('HtmlNavigateToNewMap',{'latitude': latitude,'longitude': longitude,'logo':logo,'name':name});
		},
	    type: 'iOS'
	}
    Rongyi.getCityName();
...
$(function(){
	$(".vote li span.radio").each(function(i){
		$(this).click(function(){
			if($(this).attr("isVote")=="false"||$(this).attr("isVote")==""){
