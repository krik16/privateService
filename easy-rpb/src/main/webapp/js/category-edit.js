...
//	var contactStr = ($(this).find("select[name=numberType] option:selected").val() );	
	...
	if($.trim(filename)==""){
		...
		paramsJson_['filename'] = filename;
	}


	
	if($.trim(contactStr)=="0"){
		_util.cmsTip("请选择品类级别!");
		
		return false;
	}else{
		if($.trim(contactStr)!="1" && $.trim(parentid)== "" ){
			_util.cmsTip("请填入父级品类编号!");
			return false;
		}
		paramsJson_['type'] = $.trim(contactStr);
		paramsJson_['parentid'] = $.trim(parentid);
	}
	var checklist = document.getElementsByName("selected");
	var columnids = "";
	for (var i = 0 ;i < checklist.length;i++){
		if (checklist[i].checked == 1) {
			columnids = columnids + checklist[i].value +",";
		}
	}
	paramsJson_['columnids'] = $.trim(columnids);
	var url_ = "../commoditycategory/save";
