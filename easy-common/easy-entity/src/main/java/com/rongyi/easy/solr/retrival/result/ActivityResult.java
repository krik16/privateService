...
public class ActivityResult extends Result  implements Serializable{

	/**
	 * 
	 */
	private static final long serialVersionUID = 6644543822336030147L;
	private static int count = 0;
	private List<ActivityData> result;
	private List<ActivityData> applyShop;
	private List<ActivityData> otherActivity;
	private Info info;
	public List<ActivityData> getResult() {
		return result;
	}
	public void setResult(List<ActivityData> data) {
		this.result = data;
	}
	public Info getMeta() {
		return info;
...
//			poiData.setHighlight("null");
			...
			if(record.getThumbnail()!=null){
				poiData.setThumbnail("http://rongyi.b0.upaiyun.com"+record.getThumbnail());
			}
			if(record.getCarouselImg()!=null && record.getCarouselImg().size()>0){
			...
			if(record.getHolderType() != null && record.getHolderType().equals("Mall"))
