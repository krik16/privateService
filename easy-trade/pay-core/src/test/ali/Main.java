...
import com.rongyi.pay.core.ali.model.hb.*;
import com.rongyi.pay.core.ali.model.result.AlipayF2FPayResult;
import com.rongyi.pay.core.ali.model.result.AlipayF2FPrecreateResult;
...
import com.rongyi.pay.core.ali.service.AlipayMonitorService;
import com.rongyi.pay.core.ali.service.AlipayTradeService;
import com.rongyi.pay.core.ali.service.impl.AlipayMonitorServiceImpl;
import com.rongyi.pay.core.ali.service.impl.AlipayTradeServiceImpl;
import com.rongyi.pay.core.ali.service.impl.AlipayTradeWithHBServiceImpl;
...
public class Main {
    ...
    static {
         ...
         */
         aliConfigure = AliConfigure.init();

        /** 使用Configs提供的默认参数
         *  AlipayTradeService可以使用单例或者为静态成员对象，不需要反复new
         */
        tradeService = new AlipayTradeServiceImpl.ClientBuilder().build(aliConfigure);


        // 支付宝当面付2.0服务（集成了交易保障接口逻辑）
        tradeWithHBService = new AlipayTradeWithHBServiceImpl.ClientBuilder().build(aliConfigure);
...
//        main.test_trade_precreate();
        main.test_auth_token();

    }

    //获取商户授权令牌
    public void test_mch_auth_token() throws Exception{
        AlipayClient alipayClient = new DefaultAlipayClient("https://openapi.alipay.com/gateway.do","2014020800003041", aliConfigure.getPrivateKey(),
                "json","GBK", aliConfigure.getAlipayPublicKey());
        AlipayOpenAuthTokenAppRequest request = new AlipayOpenAuthTokenAppRequest();
        request.setBizContent("{" +
                "    \"grant_type\":\"authorization_code\"," +
                "    \"code\":\"e53c586feaef409183ea035c1b0e6X24\"," +
                "  }");
        AlipayOpenAuthTokenAppResponse response = alipayClient.execute(request);
        //TODO 第一次获取到授权令牌保存到数据库，记录令牌有效时间，如果令牌有效时间快过期，刷新令牌有效期，更新令牌
        System.err.println("response=" + response);
    }
      //获取用户授权令牌
    public void test_auth_token() throws Exception{
        AlipayClient alipayClient = new DefaultAlipayClient("https://openapi.alipay.com/gateway.do","2014020800003041", aliConfigure.getPrivateKey(),
                "json","GBK", aliConfigure.getAlipayPublicKey());
        AlipaySystemOauthTokenRequest request = new AlipaySystemOauthTokenRequest();
        request.setCode("28323086bdc14a04ba901314aa44XB24");
    ...
    //一码多付方案
    public void test_scanPay(){

        //https://doc.open.alipay.com/docs/doc.htm?&docType=1&articleId=105672  支付宝一码多付方案地址
...
//         用户授权文档https://doc.open.alipay.com/docs/doc.htm?spm=a219a.7629140.0.0.ATbirE&treeId=193&articleId=105238&docType=1
...
//
//        4.你使用auth_code参数去换取user_id和access_token (access_token你用不上)
//
//        5.使用access_token可以去获取用户授权信息（这一步你也用不上，授权范围为auth_userinfo时使用）
//
//        拿到user_id后可以给下一步下单使用。
//
//        服务端拿到user_id后，user_id可以存session，也可以返回给用户页面隐藏表单，给用户页面显示付款页面。

        //总结：当面付功能中无论用户付款码支付还是用户扫码支付都需要获取授权信息。
//        付款码支付中收款到商户需要引导用户将当面付功能授权给容易网，授权地址链接可参考格式:
// http://openauth.alipay.com/oauth2/appToAppAuth.htm?app_id=2014020800003041&redirect_uri=http%3A%2F%2F120.25.197.98%3A8090%2FfuwuchuanDemo%2Fgateway.do
//        redirect_uri为我们服务器端提供的用户回调页面，商户授权页面的回调只需要根据回调的auth_code获取到对应的auth_token之后验证系统保存的
//        auth_token是否已过期，如果过期则更新，否则不变
//        用户扫码支付需要先根据用户的授权信息拿到user_id，也是通过redirect_uri实现，可在给用户的授权url中增加参数区分是商户授权还是用户付款授权,
//         用户授权链接可参考格式:https://openauth.alipay.com/oauth2/publicAppAuthorize.htm?app_id=APPID&scope=SCOPE&redirect_uri=ENCODED_URL,
//        可指定scope=auth_base实现静默授权，授权后拿到用户user_id后直接跳转到支付页面，用户输入金额后通过下单接口完成下单(https://doc.open.alipay.com/docs/api.htm?spm=a219a.7395905.0.0.wpknf5&docType=4&apiId=1046)，
//        返回下单接口获得的交易号给前端，前端发起调用用户支付宝客户端接口发起支付，用户输入密码完成支付。
...
//
    ...
    public void test_monitor_schedule_logic() {
        ...
        demoRunner.schedule(aliConfigure);
...
//                .setExceptionInfoList(exceptionInfoList) // 填写异常信息，如果有的话
    ...
    public void test_trade_pay(AlipayTradeService service) {
        ...
        String authCode = "283944590471573213"; // 条码示例，286648048691290423
...
//        String discountableAmount = "1.00"; //
        ...
        String sellerId = "2088021177665467";
...
//        String appAuthToken = "201610BB507cd9181c4942aab24f88ebeece0X48";//根据真实值填写
        ...
        // 调用tradePay方法获取当面付应答
        AlipayF2FPayResult result = service.tradePay(builder,aliConfigure);
...
//                .setNotifyUrl("http://www.test-notify-url.com")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
        ...
        AlipayF2FPrecreateResult result = tradeService.tradePrecreate(builder,aliConfigure);
        switch (result.getTradeStatus()) {
            case SUCCESS:
                log.info("支付宝预下单成功: )");
