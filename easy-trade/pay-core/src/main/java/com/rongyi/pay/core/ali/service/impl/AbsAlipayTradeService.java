...
import com.rongyi.pay.core.ali.service.request.AlipayTradeCreateRequest;
...
abstract class AbsAlipayTradeService extends AbsAlipayService implements AlipayTradeService {
    ...
    @Override
    public AlipayF2FQueryResult queryTradeResult(AlipayTradeQueryRequestBuilder builder,AliConfigure aliConfigure) {
    ...
    @Override
    public AlipayF2FRefundResult tradeRefund(AlipayTradeRefundRequestBuilder builder,AliConfigure aliConfigure) {
        validateBuilder(builder);
    ...
    @Override
    public AlipayF2FPrecreateResult tradePrecreate(AlipayTradePrecreateRequestBuilder builder,AliConfigure aliConfigure) {
        validateBuilder(builder);
    ...
    @Override
    public AlipayF2CreateResult tradeCreate(AlipayTradeCreateRequestBuilder builder,AliConfigure aliConfigure){
        validateBuilder(builder);
        ...
        AlipayF2CreateResult result = new AlipayF2CreateResult(response);
        if (response != null && Constants.SUCCESS.equals(response.getCode())) {
            // 预下单交易成功
            result.setTradeStatus(TradeStatus.SUCCESS);
    ...
    protected AlipayF2FPayResult checkQueryAndCancel(String outTradeNo, String appAuthToken, AlipayF2FPayResult result,
                                                   AlipayTradeQueryResponse queryResponse,AliConfigure aliConfigure) {
        ...
        AlipayTradeCancelResponse cancelResponse = cancelPayResult(builder,aliConfigure);
    ...
    // 轮询查询订单支付结果
    protected AlipayTradeQueryResponse loopQueryResult(AlipayTradeQueryRequestBuilder builder,AliConfigure aliConfigure) {
        AlipayTradeQueryResponse queryResult = null;
        for (int i = 0; i < aliConfigure.getMaxQueryRetry(); i++) {
            Utils.sleep(aliConfigure.getQueryDuration());
    ...
    // 根据外部订单号outTradeNo撤销订单
    protected AlipayTradeCancelResponse cancelPayResult(AlipayTradeCancelRequestBuilder builder,AliConfigure aliConfigure) {
        ...
        if (needRetry(response)) {
            ...
            asyncCancel(builder,aliConfigure);
        }
        return response;
    }

    // 异步撤销
    protected void asyncCancel(final AlipayTradeCancelRequestBuilder builder,final AliConfigure aliConfigure) {
        executorService.submit(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < aliConfigure.getMaxCancelRetry(); i++) {
                    Utils.sleep(aliConfigure.getCancelDuration());
