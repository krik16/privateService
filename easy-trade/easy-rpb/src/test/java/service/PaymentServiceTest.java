...
//		List<PaymentEntity> list = paymentService.getOrderNumStrsByPayNo("1000001597030813",Constants.PAYMENT_TRADE_TYPE.TRADE_TYPE0);
    ...
    @Test
    @Rollback(false)
    public void testInsertRepeatPay() {
        try {
            PaymentEntity paymentEntity = new PaymentEntity();
            paymentEntity.setPayNo("1234567");
            PaymentLogInfo paymentLogInfo = new PaymentLogInfo();
            paymentLogInfo.setTrade_no("1234567");
            paymentService.insertRepeatPay(paymentEntity, paymentLogInfo);
        } catch (Exception e) {
            e.printStackTrace();
        }
