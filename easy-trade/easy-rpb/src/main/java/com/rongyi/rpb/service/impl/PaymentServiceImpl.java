	...
	private Map<String, Object> getBodyMap(PaymentEntityVO paymentEntityVO, MessageEvent event) {
		...
		try {
			if (PaymentEventType.APP.equals(event.getType())) {// 手机APP支付
				...
				bodyMap = webPageAlipayService.getToken(paymentEntityVO.getPayNo(), paymentEntityVO.getAmountMoney().toString(), paymentEntityVO.getTitle());
				LOGGER.info("支付宝网页支付");
			} else if (PaymentEventType.WEIXIN_PAY.equals(event.getType())) {// 微信支付
				bodyMap = weixinPayService.getAppWeXinSign(paymentEntityVO.getPayNo(), Double.valueOf(paymentEntityVO.getAmountMoney().toString()));
				LOGGER.info("微信支付");
