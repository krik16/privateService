...
<body>
	<div class="coupon-min-width">
		<div class="coupon">
			<div class="coupon-title">商家服务>提现申请详情</div>
					...
					<div class="coupon-new">
						...
						<div class="w_100">
							<ul>
								...
								<li class="li2"><span class="msg">${apply.drawNo}</span></li>
								...
								<li class="li2"><c:choose>
										...
										<c:when test="${apply.payType==2 }">微信</c:when>
										<c:when test="${apply.payType==3 }">未知</c:when>
									</c:choose></li>
								<li class="li1">卖家账号：</li>
								<li class="li2">${apply.sellerAccount }</li>
								<li class="li1"> <c:choose>
										<c:when test="${apply.payType==0 }">卖家支付宝账号：</c:when>
										<c:when test="${apply.payType==1 }">卖家银行卡账号：</c:when>
										<c:when test="${apply.payType==2 }">卖家微信账号：</c:when>
										<c:when test="${apply.payType==3 }">未知</c:when>
									</c:choose>
								</li>
							...
							<ul>
								...
								<li class="li2"><fmt:formatDate value='${apply.createAt}'
								...
								<li class="li1"> <c:choose>
										<c:when test="${apply.payType==0 }">卖家支付宝姓名：</c:when>
										<c:when test="${apply.payType==1 }">卖家银行卡姓名：</c:when>
										<c:when test="${apply.payType==2 }">卖家微信姓名：</c:when>
									</c:choose> 
								</li>
