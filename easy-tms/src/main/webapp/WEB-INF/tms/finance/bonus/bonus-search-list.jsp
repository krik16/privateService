...
<div class="shopSelectList" id="shopSelectList">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-size: 12px;">
		<thead>
		 <tr>
		 <c:if test="${checked==1 }">
		 	<td style="text-align: center;" id="td-select-all">全选<input type="checkbox" id="select-all"/></td>
		 </c:if>
		...
		<c:choose>
			<c:when test="${not empty list}">
			  	<c:forEach var="item" items="${list}" varStatus="status">
			 		...
			 		<td style="text-align: center;"><a href="${ctx}/bs/detail?id=${item.id}&module=finance" target="mainFrame" style="text-decoration: underline;">${item.sdNo}</a></td>
			 		...
			 		<td style="text-align: center;" name="operate">
			 			<sec:authorize ifAnyGranted="TMS_F_EXM_EDIT"><a href="${ctx}/bonus/edit?id=${item.id}" class="btnsearch" >修改</a></sec:authorize>
			 			<sec:authorize ifAnyGranted="TMS_F_EXM_DELETE"><a href="javascript:void(0);" class="btnsearch" id="checkok-${item.id }" onclick="delete(${item.id });">删除</a></sec:authorize>
			 			<sec:authorize ifAnyGranted="TMS_F_EXM_VERIFY"><a href="javascript:void(0);" class="btnsearch" id="checkok-${item.id }" onclick="checkApplys(${item.id},2)">审核通过</a></sec:authorize>
			 			<sec:authorize ifAnyGranted="TMS_F_EXM_VERIFY">	<a href="javascript:void(0);" class="btnsearch" id="checkok-${item.id }" onclick="checkUnpass(${item.id });">审核不通过</a></sec:authorize>
			 		</td>
			 		</c:if>
					</tr>
			 	</c:forEach>
